...
/// before that). CitrateMemberSBT is a plain CREATE deploy, so it is
/// deployer-nonce-derived and MOVES every re-roll. Re-pin from
/// `contracts/addresses/40204.json`, not from a projection.
///
